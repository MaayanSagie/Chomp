...
#     # player2 = Random_Agent(player=2, env=env)#, train=True, random=0)   #0.1
#     # player2= DQN_Agent(player=2, env=env,parametes_path=path_load)
...
#     best_res = -200
#     loss_count = 0
#     tester = Tester(player1=Random_Agent(player=1, env=env), env=env, player2=player2)
...
#         step=1
#         action_1 = player1.get_action(state=state_1,env=env)
#         state_2 = env.get_next_state(state=state_1, action=action_1)
#         while not env.is_over(state_1):
#             step +=1
#             # Sample Environement
#             action_2 = player2.get_action(state_2, epoch=epoch,env = env)
